...
import { getPlayerListIDForTable, getScheduledTableMatches, setScheduledTableMatchToCurrentMatch } from '../functions/tables';
...
function CompactOptionButton({ icon = null, label, onPress, selected = false }) {
...
function getWizardSteps(isSingles, isScheduling) {
...
function ScheduledMatchCompactItem({ index, item, onConfirm, tableID }) {
    ...
    async function confirmScheduledMatch() {
        ...
        await setScheduledTableMatchToCurrentMatch(tableID, match.matchID, scheduledMatchID);
        ...
        >
            <View flex={1} marginRight={2}>
                ...
                <Text color={"gray.500"} fontSize={"2xs"} marginTop={1}>
                    {formatScheduledMatchTime(match.startTime)}
                </Text>
            </View>
            {showConfirm ? (
                <View flexDirection={"row"}>
...
function WizardPlayerSelector({ matchProps, onChangePlayer, playerField, selectedPlayer }) {
        ...
        async function loadImportPlayers() {
            ...
            try {
                ...
                if (matchProps.isTeamMatch) {
                    playerList = await getImportTeamMembersList(playerField, matchProps.teamMatchID);
                }
                else {
    ...
    function selectImportPlayer(playerID, player) {
        const playerForMatch = normalizePlayerForMatch({ ...player, id: playerID }, true);
        ...
        <View>
            ...
            >
                ...
                >
                    <View
                        alignItems={"center"}
                        backgroundColor={selectedPlayer?.jerseyColor || "white"}
                        borderColor={"gray.200"}
                        borderRadius={999}
                        borderWidth={1}
                        height={30}
                        ...
                        width={30}
                    >
                        <FontAwesome5 name="user" size={12} color={selectedPlayer?.jerseyColor ? "#FFFFFF" : "#71717A"} />
                    </View>
                    <View flex={1}>
                        <Text color={"gray.500"} fontSize={"2xs"} fontWeight={"bold"} textTransform={"uppercase"}>
                            Selected player
                ...
                >
                    <View
                        backgroundColor={selectedPlayer?.jerseyColor || "gray.200"}
                        borderColor={"gray.300"}
                        borderRadius={999}
                        borderWidth={1}
                        height={14}
                        ...
                        width={14}
                    />
                    <Text color={"blue.700"} fontSize={"2xs"} fontWeight={"bold"}>
                        Color
                    </Text>
                </Pressable>
            </View>
            ...
            {loadingImportPlayers ? (
                ...
                <View>
                    ...
                    {entryMode === "list" && hasImportPlayers ? (
                        <Pressable
                        ...
                        >
                            <View flex={1}>
                                <Text color={"gray.900"} fontSize={"sm"} fontWeight={"bold"} numberOfLines={1}>
                                    {selectedPlayer?.isImported && selectedName ? selectedName : "Search linked player list"}
                                </Text>
                                <Text color={"gray.500"} fontSize={"2xs"} marginTop={0.5}>
                                    {importPlayers.length} available
                                </Text>
                            </View>
                            <Ionicons name="chevron-forward" size={16} color={openScoreboardColor} />
                        </Pressable>
                    ) : (
                        <View flexDirection={"row"}>
                            <Input
            ...
            <JerseyColorModal
                color={selectedPlayer?.jerseyColor || ""}
...
export function MatchSetup(props) {
    ...
    const canUseScheduledMatches = !props.isTeamMatch && !props.isScheduling;
    ...
    const wizardSteps = getWizardSteps(isSingles, props.isScheduling);
    ...
    }, [canUseScheduledMatches, props.route?.params?.tableID])
                    ...
                    {loadingScheduledMatches && !scheduledMatchesLoaded ? (
                        ...
                        <View>
                            ...
                            <Text color={"blue.800"} fontSize={"xs"} fontWeight={"bold"} marginTop={2}>
                                Scheduled matches are ready for this table. Use one when possible to keep the schedule accurate.
                            </Text>
                        </View>
                    ) : (
                        <View backgroundColor={"gray.50"} borderColor={"gray.200"} borderRadius={8} borderWidth={1} padding={3}>
